        ...
        <div className="grid sm:grid-cols-2 justify-center gap-10 w-[85%]">
          {benefits.slice(0, 4).map((data, index) => (
            <div key={index} className="text-center bg-white p-4 sm:p-[3.75rem]">
              <div className="pt-[6px] px-[16px] w-fit mx-auto mb-5 bg-[#D9D5CC] text-[1.625rem] font-trajan leading-normal">
                {index + 1}
              </div>
              <h4 className="text-[1.75rem] leading-normal font-trajan">{data.title}</h4>
              <p className="text-xl leading-loose mt-2 text-[#585451]">{data.desc}</p>
            </div>
          ))}
        </div>
        {benefits[4] && (
          <div className="text-center bg-white p-[3.75rem] w-[85%] mt-10">
            <div className="pt-1 px-[10px] w-[3.125rem] h-[3.125rem] mx-auto mb-5 bg-[#D9D5CC] text-[1.625rem] font-trajan leading-normal">
              5
            </div>
            <h4 className="text-[1.75rem] leading-normal font-trajan">{benefits[4].title}</h4>
            <p className="text-xl leading-loose mt-2 max-w-[1200px] mx-auto text-[#585451]">{benefits[4].desc}</p>
          </div>
        )}
      </div>
    </div>
